	...
	for ( int i = 0; i < static_cast<int>( Text.length() ); ++i ){
		...
		if ( RenderState->IsOutLine || RenderState->IsBold ) {
			for ( int i = 0; i < 4; ++i ) {
				...
				if ( OutLineState.IsOutLine ) {
					OutLineState.Color.x = OutLineState.OutLineColor.x;
					OutLineState.Color.y = OutLineState.OutLineColor.y;
					OutLineState.Color.z = OutLineState.OutLineColor.z;
				}

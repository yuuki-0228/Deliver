...
void CSprite::RenderUI( SSpriteRenderState* pRenderState )
{
	...
	if ( m_IsAnimPlay ) {
		...
		AnimState->SetPatternNo();		// パターンNoを設定.
	...
	// 使用するシェーダの登録.
	const int PSShaderNo = RenderState->pRuleTexture == nullptr ? RenderState->pMaskTexture == nullptr ? 0 : 1 : 2;
			...
			0, D3D11_MAP_WRITE_DISCARD, 0, &pData ) ) )
	{
		...
		// 描画するエリア.
		D3DXVECTOR4 RenderArea = RenderState->RenderArea;
		const float WndScalseW = CDirectX11::GetWndWidth()  / FWND_W;
		const float WndScalseH = CDirectX11::GetWndHeight() / FWND_H;
	...
	m_pContext->PSSetShaderResources( 1, 1, &RenderState->pMaskTexture );
	m_pContext->PSSetShaderResources( 2, 1, &RenderState->pRuleTexture );
...
void CSprite::Render3D( SSpriteRenderState* pRenderState, const bool IsBillBoard )
{
	...
	SSpriteAnimState*	AnimState	= &RenderState->AnimState;
#ifdef _DEBUG
	CPositionRender::Render( RenderState->Transform );
#endif
	// 非表示の場合は処理を行わない.
	if ( m_IsAllDisp			== false	) return;
	if ( RenderState->IsDisp	== false	) return;
...
void CSprite::MoveUIAnimUpdate( SSpriteRenderState* pRenderState )
{
	...
	if ( Speed.x != 0.0f ) {
		if ( UIAnimState->Flag_x  )	Pos->x = CEasing::Easing( XEasingName, UIAnimState->Cnt_x, 100.0f, Back.x, StartPos.x,				StartPos.x + EndPos.x );
		else						Pos->x = CEasing::Easing( XEasingName, UIAnimState->Cnt_x, 100.0f, Back.x, StartPos.x + EndPos.x,	StartPos.x );
	}
	if ( Speed.y != 0.0f ) {
		if ( UIAnimState->Flag_y )	Pos->y = CEasing::Easing( YEasingName, UIAnimState->Cnt_y, 100.0f, Back.y, StartPos.y,				StartPos.y + EndPos.y );
		else						Pos->y = CEasing::Easing( YEasingName, UIAnimState->Cnt_y, 100.0f, Back.y, StartPos.y + EndPos.y,	StartPos.y );
	}
...
void CSprite::MoveAdvancedSetting( SSpriteRenderState* pRenderState )
{
	...
	CImGuiManager::InputVec2( u8"移動距離", &MoveEndPos );

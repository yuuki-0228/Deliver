...
struct MY_SKINMATERIAL
{
	...
	std::vector<ID3D11ShaderResourceView*>	pLightTexture;		// ライトテクスチャ.
	...
	~MY_SKINMATERIAL()
	{
		const int tSize = static_cast<int>( pTexture.size() );
		...
		const int lSize = static_cast<int>( pLightTexture.size() );
		for ( int i = lSize - 1; i >= 0; --i ) {
			if ( pLightTexture[i] != nullptr )
			{
				pLightTexture[i]->Release();
				pLightTexture[i] = nullptr;
			}
		}
	}
};

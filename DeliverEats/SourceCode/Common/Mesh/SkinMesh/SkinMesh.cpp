...
CSkinMesh::CSkinMesh()
	...
	, m_FilePath				()
	, m_iFrame					()
	, m_PointLightIntensity		( 2.0f )
	, m_PointLightBrightness	( 0.0f )
	, m_pFog					( nullptr )
	, m_IsFog					( false )
	, m_DitherFlag				( false )
...
HRESULT CSkinMesh::CreateAppMeshFromD3DXMesh( LPD3DXFRAME p )
{
	...
	for( DWORD i=0; i<pAppMesh->dwNumMaterial; i++ )
	{
		...
		if ( pAppMesh->pMaterial[i].TextureName != nullptr ) {
			if ( FAILED(
				CDirectX11::MutexD3DX11CreateShaderResourceViewFromFile(
			...
			if ( FAILED(
				...
				nullptr, nullptr, &pAppMesh->pMaterial[i].pLightTexture[0], nullptr ) ) )
			{
				pAppMesh->pMaterial[i].pLightTexture[0] = nullptr;
			}
			...
			while ( 1 ) {
				...
				// ライトテクスチャパスの作成.
				std::string LightTexName = TexName;
				size_t		LightIndex	 = LightTexName.find( "." );
				LightTexName.insert( LightIndex, "_light" );

				// ライトテクスチャを作成できるか.
				ID3D11ShaderResourceView* pLightTexture = pAppMesh->pMaterial[i].pLightTexture[0];
				CDirectX11::MutexD3DX11CreateShaderResourceViewFromFile(
					StringConversion::to_wString( LightTexName ).c_str(),
					nullptr, nullptr, &pLightTexture, nullptr );

				// 次のテクスチャがあるか確かめる.
				TexNo++;
				pAppMesh->pMaterial[i].pTexture.emplace_back(		pTexture );
				pAppMesh->pMaterial[i].pLightTexture.emplace_back(	pLightTexture );
			}
		}
	...
	STransform* pTransform, SKIN_PARTS_MESH* pMesh, D3DXMATRIX World, MYMESHCONTAINER* pContainer )
{
			...
			D3D11_MAP_WRITE_DISCARD, 0, &pData ) ) )
	{
		...
		cb.vPointLightBrightness.x	= m_PointLightBrightness;
	...
	for( DWORD i=0; i<pMesh->dwNumMaterial; i++ )
	{
		...
		if( pMesh->pMaterial[i].TextureName != nullptr )
		{
			...
			m_pContext->PSSetShaderResources( 1, 1, &pMesh->pMaterial[i].pLightTexture[m_MaterialTextureNo[i]]	);
		}
		else
		{

...
namespace {
	...
	constexpr int	DIRE_NUM			= 4;							// 部屋を表示する方向の数.
	constexpr int	ONE_DIRE_ROOM_NUM	= 3;							// 1方向の部屋の数.
	constexpr int	ROOM_NUM			= DIRE_NUM * ONE_DIRE_ROOM_NUM;	// 部屋の数.

	constexpr char	ROOM_SET_POS_BONE_NAME[]	= "apart_3x3_";			// 部屋を配置するボーン名("_"の後ろに1~12の数字を組み合わせる).
	constexpr char	NEXT_FLOOR_POS_BONE_NAME[]	= "apart_3x3_up";		// 次の部屋の乗せるのボーン名.
}

CATypeApartmentFloor::CATypeApartmentFloor()
...
// 初期化.
//---------------------------.
bool CATypeApartmentFloor::Init()
{
	// ライトをつける時間の設定.
	m_LightHour		= 16;
	m_LightMinute	= CRandom::GetRand( 1, 2 );
...
// 更新.
//---------------------------.
void CATypeApartmentFloor::Update( const float& DeltaTime )
{
//	m_IsDisp = true;
//	DistanceFadeUpdate( CONSTANT.START_DISTANCE + CONSTANT.FLOOR_SIZE, CONSTANT.ALPHA_DISTANCE );	// 距離によるアルファフェードの更新.
	...
	FloorDispUpdate();		// 1フロアの部屋の表示の更新.
}

//---------------------------.
// 当たり判定終了後呼び出される更新.
//---------------------------.
void CATypeApartmentFloor::LateUpdate( const float& DeltaTime )
{
	if ( m_IsEmptyFloor ) return;
...
// 描画.
//---------------------------.
void CATypeApartmentFloor::Render()
{
	if ( m_IsDisp ) {
...
//---------------------------.
void CATypeApartmentFloor::Sprite3DRender_A()
{
	if ( m_IsEmptyFloor ) return;
...
// ステージエディタのImGuiの描画.
//---------------------------.
void CATypeApartmentFloor::StageEditorImGuiRender()
{
	...
	if ( m_IsEmptyFloor != OldIsEmptyFloor ) {
		// メッシュの読み込み.
		if ( m_IsEmptyFloor )	m_pApartmentMesh = CMeshResource::GetSkin( "apart_3x3_plus_room_s" );
		else					m_pApartmentMesh = CMeshResource::GetSkin( "apartment_3x3_s" );
		...
		for ( int i = 0; i < ROOM_NUM; ++i ) {
			...
			const D3DXROTATION3& Rot		= { 0.0f, Math::ToRadian( 90.0f * ( i / ONE_DIRE_ROOM_NUM ) ), 0.0f };
...
// フロアの作成.
//---------------------------.
void CATypeApartmentFloor::Create( json j )
{
	// メッシュの読み込み.
#ifdef _DEBUG
	if ( m_IsEmptyFloor )	m_pApartmentMesh = CMeshResource::GetSkin( "apart_3x3_plus_room_s" );
...
#else 
	// 全てを空部屋に変更する.
	m_pApartmentMesh = CMeshResource::GetSkin( "apartment_3x3_s" );
	if ( m_IsEmptyFloor ) {
...
// フロアの移動.
//---------------------------.
void CATypeApartmentFloor::FloorMove( const D3DXPOSITION3& NewPos )
{
	// フロアの移動.
...
// 上に設置する座標を取得する.
//---------------------------.
D3DXVECTOR3 CATypeApartmentFloor::GetSettingPosition()
{
	m_pApartmentMesh->SetTransform( m_Transform );
...
// フロアのデータの取得.
//---------------------------.
json CATypeApartmentFloor::GetFloorData()
{
...
// アルファの設定.
//---------------------------.
void CATypeApartmentFloor::SetAlpha( const float& Alpha )
{
	m_Alpha = Alpha;
...
// 1フロアの部屋の表示の更新.
//---------------------------.
void CATypeApartmentFloor::FloorDispUpdate()
{
	const D3DXPOSITION3& CameraPos = CCameraManager::GetPosition();

	// カメラからの距離を求める.
	std::vector<std::pair<float, std::pair<CApartmentEmptyRoom*, std::pair<CApartmentRoom*, CApartmentShop*>>>> LengthList;
	LengthList.reserve( ROOM_NUM );
	for ( auto& [Room, No] : m_pEmptyRoomList ) {
		const D3DXVECTOR3& Vector = Room->GetPosition() - CameraPos;
		LengthList.emplace_back( std::make_pair( D3DXVec3Length( &Vector ), std::make_pair( Room.get(), std::make_pair( nullptr, nullptr ) ) ) );
	}
	for ( auto& [Room, No] : m_pRoomList ) {
		const D3DXVECTOR3& Vector = Room->GetPosition() - CameraPos;
		LengthList.emplace_back( std::make_pair( D3DXVec3Length( &Vector ), std::make_pair( nullptr, std::make_pair( Room.get(), nullptr ) ) ) );
	}
	for ( auto& [Shop, No] : m_pShopList ) {
		const D3DXVECTOR3& Vector = Shop->GetPosition() - CameraPos;
		LengthList.emplace_back( std::make_pair( D3DXVec3Length( &Vector ), std::make_pair( nullptr, std::make_pair( nullptr, Shop.get() ) ) ) );
	}
	...
	// 表示状況を切り替える.
	const int& DispNum = ROOM_NUM / 2 + ONE_DIRE_ROOM_NUM / 2;
	for ( int i = 0; i < ROOM_NUM; ++i ) {
		CApartmentEmptyRoom* pEmptyRoom	= LengthList[i].second.first;
		CApartmentRoom*		 pRoom		= LengthList[i].second.second.first;
		CApartmentShop*		 pShop		= LengthList[i].second.second.second;
		if ( pEmptyRoom != nullptr ) pEmptyRoom->SetIsDisp( i < DispNum ? true : false );
		if ( pRoom		!= nullptr ) pRoom->SetIsDisp(		i < DispNum ? true : false );
		if ( pShop		!= nullptr ) pShop->SetIsDisp(		i < DispNum ? true : false );
	}
}
...
// ライトの更新.
//---------------------------.
void CATypeApartmentFloor::LightUpdate()
{

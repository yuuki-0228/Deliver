...
cbuffer per_frame		: register( b2 )
{
	...
	float4 g_PointLightBrightness;	//メッシュに対してのポイントライトの明るさ.
...
float4 PS_Main( PSSkinIn input ) : SV_Target
{
	...
	// 出力用を作成.
	float4 finalColor = 
		((input.Color / 2.0f) * (1.0f - LightTexDiffuse.r + LightTexDiffuse.r *
		( 1.0f - g_PointLightBrightness.x)) + TexDiffuse / 2.0f) *				// 色を求める.
		(g_LightIntensity.x + ( g_PointLightIntensity.x - g_LightIntensity.x) * // ライトの強さを求める.
		(LightTexDiffuse.r * g_PointLightBrightness.x));						// ポイントライトの明るさを求める.
